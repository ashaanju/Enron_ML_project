...
# Starting with 19 features excluding email address and Director Fees of the person
...
features_list = ['poi','salary', 'deferral_payments', 'total_payments', 'loan_advances', 'bonus', 'restricted_stock_deferred', 'deferred_income', 'total_stock_value', 'expenses', 'exercised_stock_options', 'other', 'long_term_incentive', 'restricted_stock', 'to_messages','from_poi_to_this_person', 'from_messages', 'from_this_person_to_poi', 'shared_receipt_with_poi', "deferred_income_to_total_payments", "exercised_stock_option_to_stock_value", "shared_receipt_with_poi_to_messages", "to_poi_to_messages", "from_poi_to_messages"] 
...
param_grid1 = [{'feat__k': (5,8,10,12,15,20)}]
param_grid2 = [{'feat__k': (5,8,10,12,15,20), 'clf__min_samples_leaf' : (1,2,3),'clf__min_samples_split' : (2,3,4)}]
param_grid3 = [{'feat__k': (5,8,10,12,15,20), 'clf__C' : (0.01,0.1,1,5,10,50)}]
param_grid4 = [{'feat__k': (5,8,10,12,15,20), 'clf__n_neighbors' : (1,3,5,7,9)}]

## Gridsearch on each pipeline
grid1 = GridSearchCV(pipe1, param_grid = param_grid1, scoring = "f1_weighted", cv = 3)
grid2 = GridSearchCV(pipe2, param_grid = param_grid2, scoring = "f1_weighted", cv = 3)
grid3 = GridSearchCV(pipe3, param_grid = param_grid3, scoring = "f1_weighted", cv = 3)
grid4 = GridSearchCV(pipe4, param_grid = param_grid4, scoring = "f1_weighted", cv = 3)
...
best_pipe = Pipeline([('scale',MinMaxScaler()),('feat', SelectKBest(k = 5)),('clf', GaussianNB())])
...
print report_best
score_features= best_pipe.named_steps['feat'].scores_
for i in range(len(score_features)):
    print features_list[i+1],round(score_features[i],3)
